...
validate_exacerbation <- function(base_agents=1e4, input=NULL) {
  ...
  #---------------------------- SEX - SEVERITY --------------------------------
  #----------------------------------------------------------------------------
  Exac_per_sev_male <- matrix (NA, nrow = 4, ncol = 3)
  colnames(Exac_per_sev_male) <- c("Severity", "EPIC", "MACRO")

  total_exacs_male <- nrow(exac_events_male)

  Exac_per_sev_male[1:4,1] <- c("Mild","Moderate","Severe","Very severe")
  Exac_per_sev_male[1:4,2] <- c(nrow(mild_exac_events_male),
                                nrow(mod_exac_events_male),
                                nrow(sev_exac_events_male),
                                nrow(vsev_exac_events_male))/total_exacs_male*100
  Exac_per_sev_male[1:4,3] <- c(61.6, 13.2, 22.7, 2.0)

  df <- as.data.frame(Exac_per_sev_male)
  dfm <- melt(df[,c("Severity", "EPIC", "MACRO")],id.vars = 1)

  plot_male <-
    ggplot(dfm, aes(x = Severity, y = as.numeric(value))) +
    #scale_y_continuous(breaks = seq(0, 3, by = 0.5)) +
    #theme_tufte(base_size=14, ticks=F)  +
    geom_bar(aes(fill = variable), stat = "identity", position = "dodge") +
    ylab ("Percentage") + xlab("Male") +
    labs(caption = "Distribution of exacerbation severity for males and females")


  Exac_per_sev_female <- matrix (NA, nrow = 4, ncol = 3)
  colnames(Exac_per_sev_female) <- c("Severity", "EPIC", "MACRO")

  total_exacs_female <- nrow(exac_events_female)

  Exac_per_sev_female[1:4,1] <- c("Mild","Moderate","Severe","Very severe")
  Exac_per_sev_female[1:4,2] <- c(nrow(mild_exac_events_female),
                                  nrow(mod_exac_events_female),
                                  nrow(sev_exac_events_female),
                                  nrow(vsev_exac_events_female))/total_exacs_female*100
  Exac_per_sev_female[1:4,3] <- c(71.9, 9.1, 16.3, 0.7)

  df <- as.data.frame(Exac_per_sev_female)
  dfm <- melt(df[,c("Severity", "EPIC", "MACRO")],id.vars = 1)
  ...
  plot_female <-
    ...
    #theme_tufte(base_size=14, ticks=F)  +
    geom_bar(aes(fill = variable), stat = "identity", position = "dodge") +
    ylab ("Percentage") + xlab("Female")
  #labs(caption = "Distribution of exacerbation severity for females")

  plot <- ggarrange(plot_male,plot_female, common.legend=TRUE)
